...
population_city <- readRDS(file.path(input, "populationBR_cities_year.RDS"))
...
# First, lets get the population of each state
state_pop <- population_city |> 
  group_by(state) |> 
  summarize(pop = sum(pop_estimate)) |> 
  #add population abroad. Source: https://www.gov.br/mre/pt-br/assuntos/portal-consular/BrasileirosnoExterior.pdf 
  add_row(state = "ZZ", pop = 4598735)

justification |>
  group_by(UF) |> 
  summarize(contagem = sum(`Quantidade de justificativas`)) |> 
  left_join(state_pop, join_by(UF == state)) |> 
  mutate(contagem_prop = contagem/pop) |> 
  ggplot(aes(x=fct_reorder(UF, contagem_prop), y=contagem_prop, color=UF)) +
  geom_col()+
  scale_y_continuous(labels = percent)+
  labs (
    title = "Justifications, by state",
    x="State",
    y="Number of processes divided by population",
    caption = "* ZZ stands for voters abroad")+
